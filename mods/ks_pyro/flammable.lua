...
minetest.register_abm({
	...
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		if minetest.find_node_near(pos, 1, {"group:burning"}) == nil or minetest.find_node_near(pos, 1, {"group:douse_fire"}) then
...
minetest.register_node("ks_pyro:burning", {
	...
	groups = {igniter = 1, burning = 1, tongable = 1, hot_node = 1},
})
...
minetest.register_node("ks_pyro:light_burning", {
	...
	groups = {igniter = 1, light_burning = 1, tongable = 1, falling_node = 1, hot_node = 1},
})
